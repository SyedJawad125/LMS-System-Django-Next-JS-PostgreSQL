...
# ============================================
# UPDATED RAG SERVICE (BACKWARD COMPATIBLE)
# File: apps/rag_system/services/rag_service.py
# ============================================

"""
Backward compatible wrapper for the improved RAG service.
This file maintains the old interface while using the new implementation.
"""

from typing import Dict, List, Optional
import time
from .improved_query_processor import EnhancedRAGService


class VectorStoreRAGService:
    """
    Backward compatible wrapper for EnhancedRAGService.
    Maintains the old interface so existing code doesn't break.
    """
    
    def __init__(self):
        print("🔄 Initializing VectorStoreRAGService (using enhanced backend)...")
        self.enhanced_service = EnhancedRAGService()
        
        # Expose sub-services for backward compatibility
        self.groq_service = self.enhanced_service.groq
        self.vectorstore = self.enhanced_service.vectorstore
        self.db_connector = self.enhanced_service.db
        self.query_processor = self.enhanced_service.query_processor
        
        print("✅ VectorStoreRAGService ready (enhanced)!")
    ...
    def process_query(
        ...
        """
        Process query using enhanced service.
        
        Args:
            query: User's question
            user_context: User information (optional)
            use_cache: Whether to use cache (optional)
            use_reranking: Whether to rerank results (optional, legacy param)
        ...
        result = self.enhanced_service.process_query(query, user_context)
        
        # Add backward compatibility fields
        if 'context_sources' not in result:
            result['context_sources'] = {
                'vector_store_results': len(result.get('sample_results', [])),
    ...
    def get_database_summary(self) -> Dict:
        """Get database summary (backward compatible)"""
        try:
            all_tables = self.db_connector.get_all_tables()
            user_tables = [t for t in all_tables if not any(
                skip in t.lower() for skip in ['django_', 'auth_permission', 'token_blacklist']

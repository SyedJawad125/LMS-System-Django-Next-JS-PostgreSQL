...
# # ============================================
# # UPDATED VECTOR STORE SERVICE WITH API INTEGRATION
# # File: apps/rag_system/services/vectorstore_service.py
# # ============================================

# """
# COPY THIS CONTENT to apps/rag_system/services/vectorstore_service.py
# This adds API data integration for roles/permissions
# """
...
# class VectorStoreService:
#     """Enhanced Vector Store with PostgreSQL + API integration"""
...
#     def initialize_with_database_knowledge(self, refresh: bool = False):
#         """Initialize vector store with PostgreSQL database knowledge + API data"""
#         print("📚 Loading PostgreSQL database knowledge + API data...")
...
#         # ⭐ ADD API DATA (roles/permissions)
#         try:
#             from .api_data_connector import APIDataConnector
            
#             print("🔗 Fetching API data (roles/permissions)...")
#             api_connector = APIDataConnector()
#             api_docs = api_connector.create_vectorstore_documents()
            
#             if api_docs:
#                 documentation.extend(api_docs)
#                 print(f"✅ Added {len(api_docs)} API documents")
#             else:
#                 print("⚠️ No API documents created")
...
#     # Keep all existing methods from your current vectorstore_service.py
#     # Just replace the initialize_with_database_knowledge method above
...
# • Roles include: Super, Teacher, Student, Parent
# • Each role has specific permissions
# • Permissions control what actions users can perform
# • To get role permissions, query the roles and permissions data
...
#             "exam": ["test", "assessment"],
#             "fee": ["payment", "invoice"],
#             "permission": ["access", "rights", "privileges"],
#             "role": ["user type", "access level"]
#         }
...
#         if "show" in query_lower:
#             expansions.append(query_lower.replace("show", "list"))
#         if "what permissions" in query_lower:
#             expansions.append(query_lower.replace("what permissions", "list permissions"))
#             expansions.append(query_lower.replace("what permissions", "show access rights"))
...
# ============================================
# UPDATED VECTOR STORE SERVICE (BACKWARD COMPATIBLE)
# File: apps/rag_system/services/vectorstore_service.py
# ============================================

"""
Backward compatible wrapper for OptimizedVectorStore.
This maintains the old interface while using the new optimized implementation.
"""
...
class VectorStoreService:
    """
    Backward compatible wrapper for OptimizedVectorStore.
    Use this in existing code - it will use the new optimized version.
    """
    
    def __init__(self, persist_directory: str = "./data/vectorstore"):
        print("🔄 Initializing VectorStoreService (optimized backend)...")
        
        # Use optimized vector store
        self.optimized_store = OptimizedVectorStore(persist_directory)
        
        # Expose underlying components for backward compatibility
        self.embeddings = self.optimized_store.embeddings
        self.vectorstore = self.optimized_store.vectorstore
        self.persist_directory = self.optimized_store.persist_directory
        
        # Database connector for knowledge base
        self.db_connector = DatabaseConnector()
        
        print("✅ VectorStoreService ready (optimized)!")
    ...
    def initialize_with_database_knowledge(self, refresh: bool = False):
        ...
        Args:
            refresh: If True, regenerate all documents even if store exists
        """
        print("📚 Initializing vector store with database knowledge...")
        
        # Check if already initialized
        if not refresh:
            stats = self.stats()
            if stats['total_chunks'] > 100:
                print(f"✅ Already initialized with {stats['total_chunks']} chunks")
                return
        
        print("🔄 Generating knowledge base...")
        
        # Generate rich knowledge base
        ingestion = ImprovedDataIngestion()
        documents = ingestion.generate_rich_knowledge_base()
        
        print(f"📄 Generated {len(documents)} documents")
        
        # Add to vector store with optimized chunking
        print("✂️ Adding documents with semantic chunking...")
        self.optimized_store.add_documents_optimized(documents)
        
        stats = self.stats()
        print(f"✅ Initialized with {stats['total_chunks']} chunks")
    ...
    def add_documents(self, texts: List[str], metadatas: List[Dict] = None):
        ...
        # Convert to document format for optimized store
        documents = []
        for i, text in enumerate(texts):
            metadata = metadatas[i] if metadatas and i < len(metadatas) else {}
            documents.append({
    ...
    def search(self, query: str, k: int = 10) -> List[Dict]:
        ...
        """
        # Use hybrid search from optimized store
        return self.optimized_store.search(query, k=k)
    
    def stats(self) -> Dict:
        """Get vector store statistics"""
        return self.optimized_store.stats()

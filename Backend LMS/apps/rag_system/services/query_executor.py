...
# # ============================================
# # FIXED QUERY EXECUTOR - Better SQL Validation
# # File: apps/rag_system/services/query_executor_fixed.py
# # ============================================

# """
# FIXES:
# 1. SQL validation now allows "deleted" as a COLUMN name
# 2. Better error messages
# 3. More lenient validation for common patterns
# """
...
# class QueryExecutor:
#     """
#     FIXED: Execute database queries with improved SQL validation
#     """
...
#             # Step 2: Validate SQL (FIXED)
#             is_valid, validation_error = self._validate_sql_fixed(sql)
#             if not is_valid:
#                 print(f"❌ SQL validation failed: {validation_error}")
#                 return {
#                     "success": False,
...
#     def _validate_sql_fixed(self, sql: str) -> tuple:
#         """
#         FIXED: Validate SQL with proper handling of "deleted" column
        
#         The problem was: "WHERE deleted = FALSE" was being flagged
#         because validation was too strict about the word "DELETE"
...
#         # Check for ACTUAL dangerous keywords (not in WHERE clauses)
#         # We need to be smarter about this
#         dangerous_patterns = [
#             (r'\bDROP\s+TABLE\b', 'DROP TABLE'),
#             (r'\bTRUNCATE\s+TABLE\b', 'TRUNCATE TABLE'),
#             (r'\bDELETE\s+FROM\b', 'DELETE FROM'),  # DELETE as a command
...
#                 return False, f"Forbidden operation: {name}"
        
#         # Allow "deleted" as a column name in WHERE clauses
#         # This is OKAY: "WHERE deleted = FALSE"
#         # This is NOT okay: "DELETE FROM table"
#         # The regex above handles this correctly
...
# if __name__ == "__main__":
#     """Test the fixed QueryExecutor"""
#     print("="*60)
#     print("Testing FIXED QueryExecutor")
#     print("="*60)
...
#         test_sqls = [
#             ("SELECT * FROM students WHERE deleted = FALSE", True, "Valid query with 'deleted' column"),
#             ("SELECT COUNT(*) FROM students WHERE (deleted = FALSE OR deleted IS NULL)", True, "Valid count with 'deleted' check"),
#             ("DELETE FROM students WHERE id = 1", False, "Invalid - DELETE command"),
#             ("SELECT * FROM students; DROP TABLE students;", False, "Invalid - SQL injection"),
#             ("SELECT * FROM students WHERE name = 'test' -- comment", False, "Invalid - SQL comment"),
#         ]
        
#         for sql, should_pass, description in test_sqls:
#             is_valid, error = executor._validate_sql_fixed(sql)

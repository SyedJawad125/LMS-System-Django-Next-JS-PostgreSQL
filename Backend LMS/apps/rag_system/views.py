...
# ============================================
# UPDATED VIEWS (ENHANCED RAG INTEGRATION)
# File: apps/rag_system/views.py
# ============================================
    ...
    def query(self, request):
        ...
        try:
            ...
            ChatHistory.objects.create(
                ...
                response=result.get('response', ''),
                context_used=result.get('context_sources', {}),
                sql_queries=[result.get('sql_executed')] if result.get('sql_executed') else [],
                tokens_used=result.get('tokens_used', 0),
                response_time=result.get('response_time', 0.0)
            )
            ...
            response_data = {
                ...
                "sample_results": result.get('sample_results', [])[:5],  # First 5 only
        ...
        try:
            ...
            for i, result in enumerate(results, 1):
                formatted_results.append({
                    ...
                    "metadata_score": round(result.get('metadata_score', 0), 3),
                    "type": result.get('metadata', {}).get('type', 'unknown'),
                    "entity": result.get('metadata', {}).get('entity', 'unknown'),
                    "table": result.get('metadata', {}).get('table_name', 'N/A'),
                    "content_preview": result.get('content', '')[:200] + "..."
            ...
            return Response({
                "query": query,
                "results_count": len(results),
                "search_method": "hybrid (vector + keyword + metadata)",
                "results": formatted_results
            }, status=status.HTTP_200_OK)
...
def initialize_vectorstore(request):
    ...
    try:
        ...
        return Response({
            "message": "Vector store initialized successfully with enhanced knowledge base",
            "stats": stats,
            "refreshed": refresh,
            "version": "enhanced_v2.0",
            "improvements": [
                "Rich, contextual knowledge base",
                "Semantic chunking by topic",
                "MPNet embeddings (better accuracy)",
...
def rag_metrics(request):
    ...
    total_data = {
        ...
        'avg_response_time': sum(m.avg_response_time for m in metrics) / len(metrics) if metrics else 0,
        'total_tokens': sum(m.total_tokens_used for m in metrics),
        'success_rate': (sum(m.successful_queries for m in metrics) / sum(m.total_queries for m in metrics) * 100) if sum(m.total_queries for m in metrics) > 0 else 0,
        'daily_metrics': serializer.data
    }

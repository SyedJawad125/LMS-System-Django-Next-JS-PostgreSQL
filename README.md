...
### Frontend
Next.js 15+ · React 19+ · TypeScript · Tailwind CSS · Shadcn UI · Redux Toolkit · TanStack Query · Axios · React Hook Form · Zod

### Database
**PostgreSQL** — with composite indexes, unique constraints, and foreign keys across all 16 apps.

### Containerization
**Docker & Docker Compose** — backend, frontend, PostgreSQL, Redis, Celery worker/beat, and Nginx all run as separate containers, orchestrated with a single `docker-compose.yml` (see [Docker Setup](#docker-setup)).
...
│   ├── notifications/
│   │   └── templates/
│   │       └── email/          # File-based HTML email templates (see Notifications & Email Templates below)
│   ├── configuration/
│   └── search/                  # RAG indexing, retrieval, and query endpoints
├── utils/
│   ├── reusable_classes.py       # TimeStamps, TimeUserStamps base models
...
│   ├── components/         # Reusable UI (tables, forms, modals, cards)
│   ├── hooks/
│   ├── services/            # Axios API layer, one module per Django app
...
Email templates are **file-based** — actual `.html` files stored under `apps/notifications/templates/email/` (e.g., `fee_reminder.html`, `welcome.html`, `otp_verification.html`, `leave_status_update.html`) rather than raw HTML strings typed directly into the database.

The `EmailTemplate` model stores the **metadata and mapping** for each template:

| Field | Purpose |
|---|---|
| `name` | Human-readable template name |
| `subject` | Email subject line |
| `code_name` | Unique key used in code to look up the template (e.g., `FEE_REMINDER`) |
| `html_template` | Path/reference to the file-based `.html` template (or a cached copy of its rendered content) |
| `alternative_text` | Plain-text fallback for email clients that don't render HTML |

**Why file-based templates:**
- Designers/developers can edit `.html` files directly with proper syntax highlighting and version control (Git diffs), instead of editing large HTML blobs in a database admin panel
- Templates use Django's template engine (`{{ variable }}` placeholders) for dynamic content — student name, invoice number, due date, OTP code, etc.
- Celery tasks render the appropriate template file (resolved via `code_name`) and send the email asynchronously through the reusable `EmailService`
...
├── password_reset.html
├── fee_reminder.html
├── leave_status_update.html
├── exam_result_published.html
└── invitation.html
```
...
| Service | Description | Port |
|---|---|---|
| `backend` | Django + Gunicorn (DRF API) | 8000 |
| `frontend` | Next.js app | 3000 |
| `db` | PostgreSQL (+ pgvector) | 5432 |
| `redis` | Redis | 6379 |
| `celery_worker` | Background jobs (notifications, reports, embedding sync) | — |
| `celery_beat` | Scheduled tasks (attendance reports, RAG reindex) | — |
| `nginx` | Reverse proxy for backend + frontend | 80 |
